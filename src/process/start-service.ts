...
export async function startService(
  ...
  if (isRunning(st)) {
    ...
    if (!await waitForExit(st, STOP_GRACE_MS)) return false;
  }

  // A config reload can drop the service while we wait above.
  if (!host.state.has(name)) return false;
  ...
  if (proxy) return proxy.ensureStarted();
  ...
  if (after.svc.type === 'api') return waitForPort(after.svc.port, { timeout: READY_TIMEOUT_MS });
  return true;
}
