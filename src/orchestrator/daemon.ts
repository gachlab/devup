...
async function bootLazy(
  ...
  for (const svc of lazy) {
    ...
    const proxy = createLazyProxy({
      ...
      onDemandStart: async () => {
        // The live config, not the one captured at boot: a runtime `debug`
        // toggle rewrites st.svc, and starting from the stale copy would drop
        // the flag and then write the stale copy back over it.
